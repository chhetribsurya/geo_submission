...
#sample_fastq_acc_df = sample_fastq_acc_df.sample(10)
...
final_xls_df = final_xls_df.reset_index().rename(columns={0: "TF_name"})
final_xls_df.to_excel(join(GEO_ID_outdir, "ENCFF_ID_final_details_w_fastqgz_v5_final.xlsx"))

final_xls_df_select = final_xls_df.set_index("TF_name").loc[:, ["chip", "control"]]
final_xls_df_uniq = final_xls_df_select.applymap(lambda x: " ".join(list(set(x))))
final_xls_df_uniq.to_excel(join(GEO_ID_outdir, "ENCFF_ID_w_TF_name_final.xlsx"))
...
#final_xls_df_uniq.to_excel(join(GEO_ID_outdir, "ENCFF_ID_final_details_w_fastqgz_v5_final_chip_control.xlsx"))

#### For the merging of the datasets w.r.t controls :
#pd.Series(df["control"].unique()).str.split()[::1].sort_values() ## order of control sample to be concatenated at the end:
df_new = final_xls_df.loc[:, ["TF_name", "control"]]
df_dict = df_new.set_index(["control"]).to_dict(orient="split")
...
GEO_ID_df_final_reset.to_pickle(join(GEO_ID_outdir, "ENCFF_ID_final_details_w_fastqgz_v5_final.pkl"))
GEO_ID_df_final_reset = pd.read_pickle(join(GEO_ID_outdir, "ENCFF_ID_final_details_w_fastqgz_v5_final.pkl"))
pickle.dump(GEO_ID_dict, open(join(GEO_ID_outdir, "ENCFF_ID_final_details_w_fastqgz_v5_dict.pkl"), "wb"))  # save it into a file named open('save.p', "wb")
GEO_ID_dict = pickle.load(open(join(GEO_ID_outdir, "ENCFF_ID_final_details_w_fastqgz_v5_dict.pkl"), "rb"))
